...
const parseCustomFormat = (pattern) => {
  const regex = /\{([^}]+)\}|\b(type|scope|name)\b/gi;
  const segments = [];
  let lastIndex = 0;
  let match;
...
const validateCustomFormatPattern = (pattern) => {
  ...
  if (!/\bname\b/i.test(pattern)) {
    return 'Pattern must contain the "name" keyword';
  }
...
const formatCustomCommitTitle = (type, name, segments, scope, customFieldValues = {}) => {
  return segments.map(seg => {
    ...
    if (seg.type === 'keyword') {
      switch (seg.keyword) {
        ...
        case 'name': return applyCasing(name, seg.case);
      }
    }
    return '';
  }).join('');
  ...
  if (config.format === 'custom' && config.customFormat) {
    ...
    if (!match) {
      const exampleTitle = formatCommitTitle(
        config.types[0].value, 'name', config.format,
